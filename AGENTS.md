...
| Safety contactor **K1** | 2 main poles rated for the *combined* cold-start inrush of both DC supplies, coil matched to the control circuit, ≥1 auxiliary NO | On hand; **not wired.** Does the mains switching so the mushroom breaks only the coil circuit, and latches so an E-stop *release* cannot restart the system (§4.8) |
| START button + isolator/OCP/RCD | NO momentary; upstream isolator, overcurrent protection, RCD per local rules | On hand; **not wired.** START is the deliberate restart action |
| Enclosure door interlock | Switch with 2 independent NC contacts, positive/direct opening (IEC 60947-5-1 Annex K), tool-required actuator | On hand; contact arrangement and positive-opening marking unconfirmed. Contact 1 in the K1 coil circuit, contact 2 in series with the arm switch (§4.5b) |
