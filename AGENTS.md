...
This project implements a stereoscopic laser-targeting system for in-flight pest control. A Raspberry Pi 5 running Raspberry Pi OS (64-bit, arm64, non-RTOS Linux kernel) controls two OV9281 global-shutter cameras, two dual-channel 12-bit MCP4922 DACs driving the X/Y galvo axes, and a 2.5W Class 4 blue laser via TTL GPIO. The intended hardware design puts three contacts in series in the laser driver's 12 V feed — a DC-rated lever arm switch, a single-NC mushroom E-stop, and a positive-opening enclosure door interlock (§4.8, §4.5b). **Nothing switches mains and nothing latches**; the residual risk that creates is owned in §4.8. **Every part is now on hand and none of it is verified:** no contact arrangement has been ringed out, no IC marking read, no passive metered, and the one-shot period has not been scoped — a measurement that §4.8 now makes a safety figure rather than a design check.
...
| Galvo power supply | Regulated bipolar ±15 VDC, sized for both axes | On hand **as part of the galvo kit, with mating cables** — open-frame board marked `HT-15V30W V2.0-181206` (30 W total, no manufacturer, I/O on pin headers). Rails, per-rail current and the `COM` pin position are unmeasured, and `COM` is not yet bonded to the logic ground the DAC pair is referenced to. The 12 V laser PSU is not a substitute |
...
| Level translation | 2 × **SN74AHCT125N**, PDIP-14 quad bus buffer — package #1 carries the four SPI signals, package #2 carries GPIO 18 alone | On hand under a marketplace brand with **no manufacturer traceability** (Editbar / Zhengzhou trading co. / SKU ED1175, non-TI logo), so the marking certifies nothing and the §9.4 **threshold test** replaces the marking check; sourcing from an authorised distributor is recommended (`HARDWARE_WIRING.md` §9.1). `AHCT` is load-bearing: `AHC`/`HC` are pin-identical with 3.5 V thresholds. The pin-identical `AHCT126` self-gated variant was considered and rejected (`HARDWARE_WIRING.md` §9.3) |
| Monostable | 74HC123 (DIP-16), manufacturer/order code unverified | Hardware pulse-duration backstop on laser TTL (§4.1) |
| Monostable timing R | 1/2 W carbon-film 220 kΩ (1Rext/Cext, **pin 15** → +5 V) | On hand; sets one-shot period with C_ext |
| Monostable timing C | 50 V monolithic-ceramic 1 µF (across 1Cext pin 14 ↔ 1Rext/Cext pin 15) | On hand; the Hitachi datasheet gives `t_W = R·C` with **no coefficient**, so the fitted parts compute to **≈220 ms**, not 99 ms; bench verification of the assembled period is mandatory |
| AND gate | SN74HC08N (DIP-14) quad 2-input AND | Gates laser TTL = GPIO18 ∧ one-shot-Q (§4.1) |
| Mains cords | Two factory-moulded power cords, one per supply | Not fitted. **Nothing in this project switches, fuses or modifies mains** — both supplies plug into an existing wall socket whose RCD and breaker are the upstream protection. The contactor architecture of earlier revisions was abandoned unbuilt (§4.8) |
| Enclosure door interlock | Switch with **1** NC contact, positive/direct opening (IEC 60947-5-1 Annex K), DC-rated for the driver current, tool-required actuator | On hand; contact arrangement, DC rating and positive-opening marking unconfirmed. In series with the arm switch on the 12 V laser feed (§4.5b). One contact suffices now that there is no separate coil circuit |
...
| E-stop | Mushroom button: **1 NO + 1 NC**, generic unmarked blocks. DC rating and Annex K direct-opening status unconfirmed | The single **NC** sits in the 12 V laser feed (§4.8). GPIO 25 (active LOW) is a divider tapped on that same conductor, not a second contact. The NO is left unwired: a NO reaches its safe state by *closing*, so a broken wire reads healthy |
...
| Resistors | 1/2 W carbon film: 220 kΩ, 10 kΩ, 3 kΩ; all on hand, counts and values unmetered | **7 × 10 kΩ are needed**: 1 arm series, 1 E-stop series, 3 laser-path fail-LOW, 2 chip-select pull-ups. 2 × 3 kΩ are the lower legs. Both sense networks are now identical and both junctions read ≈ **2.77 V** |
...
- RPi 5 GPIO 25 → a 10 kΩ/3 kΩ divider tapped on the 12 V laser feed immediately after the mushroom's NC contact (configurable via `e_stop_pin`). Not a second contact — the mushroom has only one NC and it is committed to breaking the feed itself.
...
- Galvo driver powered by a separate bipolar ±15 VDC supply; laser driver powered by 12 V from the Mean Well supply. The ±15 V supply is on hand but unmeasured, and remains a pre-power no-go item until all three conductors are checked.
...
**Hardware backstop — 74HC123 retriggerable monostable on the TTL line.** A one-shot sits between the SN74AHCT125N #2 fail-LOW GPIO 18 interface and the laser driver TTL, wired so the laser TTL is `translated GPIO18 ∧ one-shot-Q`, with channel-1 A held LOW, the translated fire line driving channel-1 B and CLR/RD, and the active-HIGH Q taken from standard DIP pin 13 into the AND gate. The one-shot triggers on GPIO 18's rising edge and re-arms/reset while the fire line is LOW. Effect: a normal short pulse passes through unchanged (the AND gate follows GPIO 18), but a control thread that hangs with GPIO 18 stuck HIGH is force-cut when Q times out — **with no software path and no operator action.** This is the enforcer the software cannot be: it is the only thing that makes the pulse-duration bound independent of the control thread. The on-hand device is a **Hitachi HD74HC123P**, whose datasheet gives `t_W = R_ext · C_ext` with **no coefficient**: the fitted `220 kΩ × 1 µF` computes to **≈220 ms**, not the 99 ms this document previously claimed from TI's `K = 0.45`. At 220 ms the one-shot sits *above* the ~105 ms software bound, so it trips only on a real control-thread failure — but holds 2.5 W on for over twice the software limit first. The actual bound remains the mandatory measured assembled period (see `docs/HARDWARE_WIRING.md` §11a). The 220 kΩ goes to **pin 15** (`1Rext/Cext`), with the capacitor between pins 14 and 15: both channels use the same order (`6 = 2Cext, 7 = 2Rext/Cext`; `14 = 1Cext, 15 = 1Rext/Cext`), and two earlier revisions had 14 and 15 the other way round, which puts the resistor on the capacitor-only node and makes the measured period meaningless.

The guarantee is real **only if three conditions hold, and each must be verified on the bench — an unverified backstop is a comment (§4, §7):**
1. **The AND gating is present.** If Q drives the laser TTL *alone* (no AND with GPIO 18), every fire pulse is stretched to the full one-shot width — ~220 ms with the parts as fitted, a hazard rather than a guard, and more than twice what the software would ever command. Verify: a short GPIO 18 pulse must produce an equally short laser pulse; a *stuck-HIGH* GPIO 18 must produce a single one-shot-width pulse and then stay dark.
...
3. **The measured period is what you intend.** As fitted the one-shot is ~220 ms, which sits *above* the ~105 ms real software bound: it never clips a legitimate pulse and trips only on a genuine control-thread failure — but it then holds 2.5 W on for more than twice the software limit. To make hardware the binding limit instead (safe, and arguably desirable, since it also clips max-length pulses) use R_ext = 100 kΩ for ~99 ms; for failure-only tripping with modest slack, 130–150 kΩ. R/C tolerance alone can swing the period ±20%, and an unrecorded ceramic dielectric far more.
...
**The enclosure door interlock rides this path, deliberately.** The door switch's NC contact sits in series with the arm switch on the 12 V laser feed, *upstream* of the GPIO 24 sense tap (`docs/HARDWARE_WIRING.md` §5, §6a). One contact is enough: with no contactor there is no second circuit to break. So GPIO 24 means "the laser driver actually has power available", not merely "the lever is up", and opening the door reads as a disarm that this existing code path already handles correctly. **There is no door GPIO and no door-specific code, and that is the design, not an omission:** an interlock's job is to remove the hazard, not to inform software, and a contact that opens the 12 V feed is strictly stronger than a pin the control thread polls. Do not add an `IDoorInterlock` and a control-loop poll — it would buy a signal that cannot do anything the contacts have not already done.
...
**Enforced by:** The `EStop` class reads a dedicated GPIO input (active LOW). The `ControlThread` checks `e_stop.is_pressed()` every cycle before any arm/fire logic. If pressed, it calls `FiringController::emergency_stop()` to force the laser off and transitions the state machine to `SAFE_HALT`, then breaks the control loop. The E-stop is independent of the arm switch and the watchdog, and it bypasses all state transitions via the `ANY → SAFE_HALT` path. GPIO read failure forces **pressed** (fail-safe).

**The mushroom breaks the laser driver's 12 V feed; GPIO 25 is a tap on that same conductor.** `docs/HARDWARE_WIRING.md` §3 puts three contacts in series in the `12 V+` line — E-stop NC, arm lever, door NC — with the driver's `+VIN` at the end. GPIO 25 is a 10 kΩ/3 kΩ divider tapped immediately *after* the mushroom; GPIO 24 is the identical divider tapped *after all three*. That difference in tap position is the only reason the software can tell an emergency (→ terminal `SAFE_HALT`) from a disarm (→ `IDLE`). A consequence worth stating: GPIO 25 also reads LOW on an unplugged or failed 12 V supply, which is correct — the system genuinely cannot fire and must not report otherwise.

**Two earlier architectures were rejected, and both reasons still bind:**

1. **Mains through the mushroom contact.** The LRS-50-12 specifies a 45 A cold-start inrush at 230 VAC. A pilot-duty contact block subjected to that **welds**, leaving an E-stop that looks and feels normal and disconnects nothing. Satisfied here: the mushroom switches a 12 V DC branch, not mains.
2. **A latching contactor K1 on the mains**, with the mushroom breaking only its coil circuit. Designed, then **abandoned unbuilt** — no contactor was obtained, and a hand-built mains control circuit is the highest-risk work in a project built by one person with no qualified electrician. A guard that exists on the bench beats a guard that exists in a document.

**What K1 bought and this does not is the latch, and the gap is this code's own doing.** `SAFE_HALT` is terminal, so the documented recovery from an E-stop is "restart the process." Releasing the mushroom restores the driver's 12 V. Nothing fires on release, because the software is in the terminal halt with GPIO 18 LOW and the §9.3 pull-downs holding the TTL line down — **unless the control thread had hung with GPIO 18 HIGH**, in which case the laser emits for one 74HC123 one-shot period before the backstop cuts it. **That makes the measured one-shot period (§4.1) a safety figure in this build, not a design check**, and it is an argument for sizing it near the ~105 ms software bound rather than the ≈220 ms the fitted parts give.

Worse, an operator who restarts the software with the arm lever still ON walks the state machine `INIT → IDLE → ARMED → TRACKING → FIRING` with no deliberate start action anywhere in the sequence.

**Required compensating control — not yet implemented.** `ArmSwitch` must observe a LOW→HIGH transition before it ever reports armed, so a process that starts with the lever already up stays disarmed until the operator cycles it. That restores in software the deliberate-start property the contactor provided in hardware, and it is testable with injected GPIO reads. Until it exists, the only thing standing in that gap is the `HARDWARE_WIRING.md` §15 operator procedure *"lever OFF before releasing the mushroom"* — a procedure, not a structural guard, and by this document's own standard (§4, §7) **that is not enforcement.** It is tracked as an open item in `docs/HARDWARE_INVENTORY.md`.

The Raspberry Pi runs from its own USB-C supply and nothing switches it, so the log, both GPIO sense circuits and the 5 V logic rail holding the §9.3 pull-downs defined all survive an E-stop. Residual risks this architecture does **not** remove: mains and both DC supplies stay live while the mushroom is pressed — it is not an isolator, and work inside the enclosure means unplugging both supplies; the galvo ±15 V is never removed; and the single NC contact carries the whole hardware interlock, so a welded contact defeats the hardware and the software sense together. The defence against that last one is **positive/direct opening action** (IEC 60947-5-1 Annex K) on that contact, and the fitted block is a generic unmarked part whose Annex K status is unconfirmed.
...
- **Arm Switch:** GPIO 24 (configurable via `arm_switch_pin`), active HIGH, ≈ 2.77 V. The same lever switch must switch 12 V power to the laser driver as a hardware interlock; verify its topology and DC rating. The E-stop and the door interlock are in series with it and the sense tap is downstream of all three, so an open door or a pressed mushroom both read as a disarm (§4.5b).
- **E-Stop:** GPIO 25 (configurable via `e_stop_pin`), active LOW. The mushroom's single NC contact breaks the **laser driver's 12 V feed**; GPIO 25 is a 10 kΩ/3 kΩ divider tapped on that same conductor immediately after the contact, reading ≈ 2.77 V when released. It therefore also goes LOW on a dead or unplugged 12 V supply, which is correct. **Nothing latches:** releasing the mushroom restores the driver's 12 V (§4.8).
